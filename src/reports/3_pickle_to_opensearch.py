...
reports_mapping = {
    "mappings": {
        "properties": {
            ...
            "release_date": {"type": "date", "format": "yyyyMMdd'T'HHmmssZ"},
        },
    },
}
...
with conn_pg.cursor() as cur:
    cur.execute(
        "SELECT id, title, issuing_organization, release_date FROM reports"
    )
...
for file in files:
    ...
    release_date = release_dates[file_id].strftime("%Y%m%dT%H%M%S%z")
